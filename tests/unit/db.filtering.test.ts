...
import type { RubricType, EvaluationWithStats } from '../../src/lib/types';
...
describe('Evaluation Filtering', () => {
  it('paginates results correctly', () => {
    ...
    const page1 = dbModule.getEvaluations({}, 10, 0);
    const page2 = dbModule.getEvaluations({}, 10, 10);
  ...
  it('filters by minimum accuracy score', () => {
    ...
    dbModule.updateResult(r1.id, { accuracy_score: 90, status: 'completed' });

    const r2 = dbModule.insertResult(lowEval.id, model.id);
    dbModule.updateResult(r2.id, { accuracy_score: 40, status: 'completed' });
