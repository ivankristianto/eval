...
describe('EvaluationExecutor', () => {
  ...
  it('handles missing result records gracefully', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(`No result record found for model ${model.id}`);
  ...
  it('handles evaluation timeouts', () => {
    ...
    const executor = new EvaluationExecutor();
    (executor as unknown as { handleTimeout: (id: string) => void }).handleTimeout(evaluationId);
