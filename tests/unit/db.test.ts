...
describe('Evaluation CRUD', () => {
  ...
  it('returns all results for an evaluation', () => {
    ...
    expect(fetched.map((item) => item.model_id).sort()).toEqual([modelA.id, modelB.id].sort());
  });
});
...
describe('Template CRUD', () => {
  ...
  it('returns a template with parsed model_ids', () => {
    const model = createModel();
    const template = dbModule.insertTemplate('Template', 'Instruction', [model.id], 'exact_match');
