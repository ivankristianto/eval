...
describe('POST /api/templates/import', () => {
  ...
  it('should reject invalid temperature values', async () => {
    ...
    expect(result.errors[0].error).toContain('temperature');
    expect(result.errors[1].error).toContain('temperature');
  });
  ...
  it('should reject invalid system prompts', async () => {
    ...
    expect(result.imported).toBe(0);
    expect(result.failed).toBe(1);
    expect(result.errors[0].error).toContain('system_prompt');
  });
