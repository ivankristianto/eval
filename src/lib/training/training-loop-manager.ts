 ...
 *
 * This replaces the deprecated IterativeTrainingLoop class with proper schema alignment.
 */
...
import { buildConfusionMatrix, calculateMetrics } from '@lib/evaluation/metrics';
...
const logger = createLogger('TrainingLoopManager');
 ...
 *
 * Uses current Persona schema:
 * - target_pass_rate (not target_f1_score)
 * - best_pass_rate (not best_f1_score)
 * - Reads prompts from task_prompt_versions and judge_prompt_versions tables
 * - Uses training_loop_state table for iteration tracking
 */
export class TrainingLoopManager {
  ...
  /**
   * Initializes a new training loop manager instance.
   * @param config - Configuration for the training loop
   * @param db - Database connection
   */
  constructor(config: TrainingLoopConfig, db: Database) {
  ...
  async execute(): Promise<void> {
    try {
      // Fetch persona details using current schema
        ...
        this.db
          .prepare(
            ...
            (session_id, persona_id, total_iterations,
             ...
             task_results_evaluated, current_iteration, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `
          )
          .run(
      ...
      // Get current iteration number from training_loop_state
      const state = this.db
        .prepare('SELECT current_iteration FROM training_loop_state WHERE session_id = ?')
        .get(this.sessionId) as { current_iteration: number } | undefined;

      const startIteration = state?.current_iteration ?? 1;
      ...
      for (
        ...
        iterationNumber <= this.maxIterations && !this.isStopped && !this.isPaused;
        iterationNumber++
      ) {
  ...
  private async runSingleIteration(
    ...
    // Get current judge prompt from versioned table
    ...
    const judgePromptText = judgePrompt?.prompt_text || '';

    // Get current task prompt from versioned table
    const taskPrompt = this.db
      .prepare(
        'SELECT prompt_text FROM task_prompt_versions WHERE persona_id = ? ORDER BY version_number DESC LIMIT 1'
      )
      .get(this.personaId) as { prompt_text: string } | undefined;

    const taskPromptText = taskPrompt?.prompt_text || '';
    ...
    await this.generateJudgeDecisions(iterationId, taskPromptText);
    ...
    // Check convergence using target_pass_rate (current schema)
   ...
   * @param taskPromptText - The task prompt to use for generation
   * @returns Promise that resolves when all decisions are generated and stored
   */
  private async generateJudgeDecisions(iterationId: string, taskPromptText: string): Promise<void> {
  ...
  private async refinePrompts(iterationId: string): Promise<void> {
    ...
    const persona = this.db
      ...
      .get(this.personaId) as { prompt_engineer_model_id: string } | undefined;
    ...
    const currentTaskPrompt = taskPromptVersion?.prompt_text || '';
    ...
    try {
      ...
      if (result.refined_task_prompt) {
        ...
        this.storeTaskPromptVersion(nextIterationNumber, currentTaskPrompt, 'ai', metrics);
      }
      ...
      this.storeJudgePromptVersion(nextIterationNumber, fallbackJudgePrompt, 'ai', metrics);
      this.storeTaskPromptVersion(nextIterationNumber, currentTaskPrompt, 'ai', metrics);
    }
  }
  ...
  private storeTaskPromptVersion(
    ...
    const existing = this.db
      .prepare('SELECT id FROM task_prompt_versions WHERE persona_id = ? AND version_number = ?')
  ...
  private storeJudgePromptVersion(
    ...
    // Check if a version already exists for this iteration
    const existing = this.db
      .prepare('SELECT id FROM judge_prompt_versions WHERE persona_id = ? AND version_number = ?')
      .get(this.personaId, iterationNumber);
  ...
  private async refinePromptsBasedOnHumanFeedback(iterationId: string): Promise<void> {
    ...
    // Get current prompts from versioned tables
    const taskPromptVersion = this.db
      .prepare(
        'SELECT prompt_text FROM task_prompt_versions WHERE persona_id = ? ORDER BY version_number DESC LIMIT 1'
      )
    ...
    try {
      ...
      this.storeTaskPromptVersion(2, currentTaskPrompt, 'ai', metrics);
